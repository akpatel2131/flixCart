...
//   if(user.address === "ADDRESS_NOT_SET"){
//     throw new ApiError(httpStatus.BAD_REQUEST, "Address is not set")
//   }

//   if(user.walletMoney === 0){
//     throw new ApiError(httpStatus.BAD_REQUEST, "Money is not enough")
//   }
...
//   // let product = await Product.findOne({email: user.email})

//   // if(!product){
//   //   throw new ApiError(httpStatus.BAD_REQUEST, "Product doesn't exist in database")
//   // }

//   let total = user.walletMoney

//   for(let i=0; i< cart.cartItems.length; i++){
//     let product = await Product.findOne({ _id: cart.cartItems[i].product._id})
...
//     total = total - product.cost * cart.cartItems[i].quantity;
//   }

//   const userData = { ...user, walletMoney: total };
//   const result =  await User.findOneAndUpdate({ _id : user._id}, userData);
//   cart.cartItems = [];
//   await cart.save();
//   return result
// }
...
const checkout = async (user) => {
  ...
  console.log(hasSetNonDefaultAddress,"hahaha")
